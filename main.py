...
# Функция для получения и загрузки картинок
def fetch_and_upload_cat_images(token, text_for_image, number_of_images,
                                uploaded_files_info, overall_progress_bar):
    for i in range(number_of_images):
        ...
        try:
            ...
            print(f"Не удалось загрузить {file_name} на Яндекс.Диск: {e}")
...
def main():
    ...
    with tqdm(total=number_of_images, desc='Загрузка изображений', unit='изображение',
        ...
        fetch_and_upload_cat_images(token, text_for_image, number_of_images,
                                    uploaded_files_info, overall_progress_bar)
    ...
    print("\nИзображения загружены на Яндекс.Диск!\n")
